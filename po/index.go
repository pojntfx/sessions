...
//go:generate sh -c "find . -name '.*' -prune -o -name '*.po' -print0 | xargs -0 -I {} msgmerge --update --backup=none \"{}\" sessions.pot"
//go:generate sh -c "find . -name '.*' -prune -o -type f -name '*.po' -print0 | xargs -0 -I {} sh -c 'mkdir -p $(basename {} .po)/LC_MESSAGES && msgfmt -o $(basename {} .po)/LC_MESSAGES/sessions.mo {}'"
//go:generate sh -c "find . -name '.*' -prune -o -name \"*.po\" -exec basename {} .po \\; > LINGUAS"
//go:embed *
var FS embed.FS
