...
//go:generate sh -c "find . -mindepth 1 -name '.*' -prune -o -name '*.po' -print0 | xargs -0 -I {} msgmerge --update --backup=none \"{}\" sessions.pot"
//go:generate sh -c "find . -mindepth 1 -name '.*' -prune -o -type f -name '*.po' -print0 | xargs -0 -I {} sh -c 'mkdir -p $(basename {} .po)/LC_MESSAGES && msgfmt -o $(basename {} .po)/LC_MESSAGES/sessions.mo {}'"
